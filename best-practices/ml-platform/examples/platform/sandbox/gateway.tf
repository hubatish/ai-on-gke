...
resource "google_compute_managed_ssl_certificate" "external_gateway" {
  ...
  name    = "${data.kubernetes_namespace_v1.team.metadata[0].name}-external-gateway"
  project = data.google_project.environment.project_id
...
resource "null_resource" "gateway_manifests" {
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      K8S_NAMESPACE  = data.kubernetes_namespace_v1.team.metadata[0].name
    }
