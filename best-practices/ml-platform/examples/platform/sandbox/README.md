...
- Open the `ml-team` Ray dashboard
  ...
  > If you get `ERR_CONNECTION_CLOSED` or `ERR_CONNECTION_RESET` when trying to go to the Ray dashboard, the Gateway is still being provisioned. Retry in a couple of minutes.

  > If you get `ERR_SSL_VERSION_OR_CIPHER_MISMATCH` when trying to go to the Ray dashboard, the SSL certificate is still being provisioned. Retry in a couple of minutes.
